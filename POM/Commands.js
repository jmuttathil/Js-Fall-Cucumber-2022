 ...
 */
  ...
  /**
   * Generic function to find if wE is Displayed
  ...
  async isWebElementSelected(locator) {
    /*
            1. find the webElement
            2. if found, check if element is selected
            3. otherwise, wait for 1-second then start from step-1
  ...
  /**
   * Generic function to wait for a new window to open
   * name: waitForNewWindow
   * input: numWindowBeforeClick
   *
   */
  async waitForNewWindow(numWindowBeforeClick) {
    let windowCount = numWindowBeforeClick;
    let allHandles = [];
    do {
      allHandles = await this.getHandles();
      windowCount = allHandles.length;
      console.log(`\n${numWindowBeforeClick}|${windowCount}`);
    } while ((windowCount = numWindowBeforeClick));
  }
  /**
   * Generic function to get window url
   * name: getWindowURL
   *
   */
  async getWindowURL() {
  ...
  /**
   * Generic function to get window title
   * name: getWindowTitle
   *
   */
  ...
  /**
   * Generic function to return disabled calendar dates
   * name: getAllDisabledDates
   * input: locator (for all dates)
   */
  async getAllDisabledDates(locator) {
    const allDisabledDates = await $$(locator);
